...
const checkNoNewMail = ()=> {
    const root = document.querySelector(':root');
    const noNewMailElement = document.querySelector('.ar4.z .TD > .TC')
    // if (noNewMailElement && noNewMailElement.innerHTML.match(/no new mail!/ig)) {
    //     // root.style.setProperty('--background', '#5a94f6');
    //     // root.style.setProperty('--checkbox-bar-visibility', 'hidden');
    // } else {
    //     // root.style.setProperty('--background', '#F2F2F2');
    //     // root.style.setProperty('--checkbox-bar-visibility', 'visible');
    // }
}
setInterval(checkNoNewMail, 1000);
